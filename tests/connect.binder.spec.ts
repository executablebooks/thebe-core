import { makeGitHubUrl, makeGitLabUrl, makeGitUrl } from "../src/connection";
...
describe("connect.binder", () => {
  describe("Building urls", () => {
    test("git", () => {
      ...
      expect(() =>
        makeGitUrl(makeBinderOptions(RepoProvider.github))
      ).toThrow();
    });
    test("github", () => {
      ...
      expect(
        makeGitHubUrl(makeBinderOptions(RepoProvider.github, "master"))
      ).toEqual(
        "https://binder.curvenote.dev/build/gh/binder-examples/requirements/master"
      );
      expect(() =>
        makeGitHubUrl(makeBinderOptions(RepoProvider.git))
      ).toThrow();
    });
    test("gitlab", () => {
      ...
      expect(
        makeGitLabUrl(makeBinderOptions(RepoProvider.gitlab, "master"))
      ).toEqual(
        "https://binder.curvenote.dev/build/gl/binder-examples%2Frequirements/master"
      );
      expect(() =>
        makeGitLabUrl(makeBinderOptions(RepoProvider.git))
      ).toThrow();
    });
  });
});
